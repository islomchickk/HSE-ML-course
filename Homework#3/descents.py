...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        # TODO: implement calculating gradient function
        rand_ind = np.random.randint(low=0, high=y.shape[0], size=self.batch_size)
        x = x[rand_ind]
        y = y[rand_ind]
        if self.loss_function is LossFunction.MSE:
            return 2 * (x.T @ ((x @ self.w) - y)) / self.batch_size
        elif self.loss_function is LossFunction.LogCosh:
            return x.T @ np.tanh(x @ self.w - y) / self.batch_size
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        eta = self.lr() * m_hat / (np.sqrt(v_hat) + self.eps)
        self.w -= eta
        return -eta
...
class BaseDescentReg(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        l2_gradient = np.append(self.w[:-1], 0)
